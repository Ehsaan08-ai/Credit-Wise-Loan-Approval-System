...
applicant_income = st.number_input(
    "Applicant Income ($)", min_value=0, max_value=1000000, value=5000, step=100
)
coapplicant_income = st.number_input(
    "Coapplicant Income ($)", min_value=0, max_value=1000000, value=0, step=100
)
loan_amount = st.number_input(
    "Loan Amount ($)", min_value=0, max_value=1000000, value=10000, step=100
)
loan_term = st.slider("Loan Term (months)", min_value=6, max_value=84, value=36, step=6)
savings = st.number_input(
    "Savings ($)", min_value=0, max_value=1000000, value=5000, step=100
)
collateral_value = st.number_input(
    "Collateral Value ($)", min_value=0, max_value=1000000, value=10000, step=100
)
...
dti_ratio = st.slider(
    "Debt-to-Income Ratio", min_value=0.0, max_value=1.0, value=0.3, step=0.01
)
existing_loans = st.slider(
    "Number of Existing Loans", min_value=0, max_value=10, value=0
)
...
employment_status = st.selectbox(
    "Employment Status", options=["Contract", "Salaried", "Self-employed", "Unemployed"]
)
...
loan_purpose = st.selectbox(
    "Loan Purpose", options=["Business", "Car", "Education", "Home", "Personal"]
)
...
if st.button("Predict Loan Approval", type="primary"):
    ...
    input_data["Employment_Status_Salaried"] = (
        1 if employment_status == "Salaried" else 0
    )
    input_data["Employment_Status_Self-employed"] = (
        1 if employment_status == "Self-employed" else 0
    )
    input_data["Employment_Status_Unemployed"] = (
    ...
    input_data["Employer_Category_Government"] = (
    ...
    input_data["Employer_Category_Unemployed"] = (
    ...
    if prediction == 1:
        st.success(
            "✅ **APPROVED** - Congratulations! Your loan application is likely to be approved."
        )
        st.info(f"Confidence: {prediction_proba[1] * 100:.1f}%")
    else:
        st.error(
            "❌ **REJECTED** - Unfortunately, your loan application is likely to be rejected."
        )
